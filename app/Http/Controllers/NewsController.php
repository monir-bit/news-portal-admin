...
class NewsController extends Controller
{
    ...
    public function user_news($user)
    {
         $news = News::orderby('date', 'DESC')->where('author_id',$user)->take(400)->get();
        return view('admin.news.index', compact('news'));
    }
    ...
    public function create()
    {
        ...
        $weare = WeAre::all();
        return view('admin.news.create', compact('categories', 'divisions', 'timelines', 'weare'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Auto generate unique random Hash ID (NanoID style) - always auto-generated, immutable
        // Hash ID cannot be manually set - it's always auto-generated
        $news->slug = \App\Models\News::generateUniqueSlug();
        $news->save();
    ...
    public function update(Request $request)
    {
     ...
     // dd($request);
        ...
        // Hash ID (slug) is immutable - never change it after creation
        // Only generate if it doesn't exist (for old news items)
        if (empty($news->slug)) {
            $news->slug = \App\Models\News::generateUniqueSlug(null, $news->id);
        }
        // Do NOT update slug if it already exists - it's immutable
        $news->save();
    ...
    public function publishadd($newsId,$type)
    {
    ...
    public function createByCategory($categoryId, $categoryName)
    {
        ...
        $divisions = $this->_helepr->getDivsions();
        $weare = WeAre::all();
        return view('admin.news.add-by-category.create', compact('weare', 'categories', 'categoryId', 'divisions', 'categoryName', 'timelines'));
    }
    ...
    public function getList($categoryId)
    {
        ...
        if ($role == 'representative') {
               return 1;
            $news = News::select('news.*')->orderby('updated_at', 'DESC')->join('news_categories', 'news.id', 'news_categories.news_id')->join('publisheds', 'publisheds.news_id', 'news.id')->join('users', 'users.id', 'publisheds.created_by')->where('news_categories.category_id', $categoryId)->where('users.id', auth()->user()->id)->take(1000)->get();
        } else {
            $news = News::select('news.*')->join('news_categories', 'news.id', 'news_categories.news_id')->where('news_categories.category_id', $categoryId)->orderby('id', 'DESC')->take(1000)->get();
        }
    ...
    public function edit($newsId, $categoryName = '')
    {
        ...
        return view('admin.news.add-by-category.edit', compact('news', 'weare', 'keyWords', 'divisions', 'newsKeywords', 'categoryName', 'timelines', 'categoryId', 'seo', 'categories', 'newsCategory', 'newsCategory', 'newsSubCategory'));
    }
